...
Adaptado para Django do parser standalone validado com 171 produtos (6 PDFs).
Mantém 100% de acurácia usando validação matemática como método primário.
...
@dataclass
class Orcamento:
    """Representa um orçamento completo extraído do PDF."""
    numero: str
    codigo_cliente: str
    nome_cliente: str
    vendedor: str
...
class PDFOrcamentoParser:
    """
    Parser principal de orçamentos PDF.

    Usa validação matemática determinística (quant × unit = total)
    como método primário de extração, com regex apenas para IDs.
    """
    ...
    def __init__(self, pdf_path: Optional[str] = None):
        ...
        Args:
            pdf_path: Caminho completo para o arquivo PDF (opcional)
        """
        self.pdf_path = pdf_path
        self.texto_completo = ""
        self.orcamento: Optional[Orcamento] = None

    def extrair_texto(self, pdf_path: Optional[str] = None) -> str:
        """
        Extrai todo o texto do PDF.

        Args:
            pdf_path: Caminho do PDF (usa self.pdf_path se não fornecido)

        Returns:
            Texto completo do PDF

        Raises:
            ParserError: Se houver erro ao ler o PDF
        """
        path = pdf_path or self.pdf_path

        if not path:
            raise ParserError("Caminho do PDF não fornecido")

        if not Path(path).exists():
            raise ParserError(f"Arquivo PDF não encontrado: {path}")
        ...
        try:
            ...
            raise ParserError(f"Erro ao extrair texto do PDF: {str(e)}")
    ...
    def encontrar_valores_deterministicos(self, texto_linha: str) -> Optional[Tuple[Decimal, Decimal, Decimal, int]]:
        ...
        ESTRATÉGIA: Busca de trás para frente (reversed)
    ...
    def extrair_descricao_produto(self, texto_completo: str, codigo: str,
        ...
        Estratégia: Remove os 3 últimos números (quant, unit, total) do final
    ...
    def processar_linha_produto(self, linha: str) -> Optional[Produto]:
        ...
        Estratégia: Usa " UN" isolado (com espaço) como marcador final
    ...
    def parse(self, pdf_path: Optional[str] = None) -> Orcamento:
        """
        ...
        Raises:
            ParserError: Se houver erro durante o parsing
        """
        # Extrai texto do PDF
