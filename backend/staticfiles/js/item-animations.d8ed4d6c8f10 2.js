...
(function() {
    ...
    function setupHTMXAnimations() {
        ...
        // Escutar evento 'itemDesmarcado' (quando item volta para não separado)
        document.body.addEventListener('itemDesmarcado', async (event) => {
            console.log('[Animations] Evento itemDesmarcado recebido', event);
            ...
            // 1. Extrair item_id do header HTTP (enviado pelo backend)
            const itemId = event.detail?.HEADERS?.['x-item-id'] ||
                           event.detail?.['x-item-id'] ||
                           event.detail?.itemId;

            if (!itemId) {
                console.error('[Animations] item_id não encontrado no evento itemDesmarcado');
                return;
            }
            ...
            try {
                ...
                const url = `/pedidos/${pedidoId}/itens/${itemId}/partial/`;
                ...
                // 6. Inserir HTML fresco no container de não-separados
                const containerNaoSeparados = document.getElementById('container-nao-separados');

                if (!containerNaoSeparados) {
                    console.warn('[Animations] Container de não-separados não encontrado, recarregando...');
                    setTimeout(() => location.reload(), ANIMATION_DURATION);
                    return;
                }

                const newItem = await insertWithAnimation(containerNaoSeparados, freshHTML);
